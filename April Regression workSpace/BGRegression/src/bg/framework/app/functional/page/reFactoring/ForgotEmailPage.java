...
// Verify whether Forgot Email Link exists and if it exists click on the Forgot Email Link and check whether it navigates to the appropriate page    
    ...
    public void validateForgotEmailNew(final UserProfile userProfile) {
        
        verifyAndClickWithXpath(pageProperties.getProperty("ForgotEmailPage.ForgotDetail"), "forgot My Details");
        browser.wait(1000);
        verifyAndClickWithXpath(pageProperties.getProperty("RegistrationPage.forgotEmail"), "forgot My Email");
...
// Verify whether appropriate fields are displayed in the Forgot Email Page     
    ...
    public void validateForgotEmailTitleNew(UserProfile userProfile) {
        
    	browser.wait(10000);
    	
    	 verifyAndSelectDropDownBox(pageProperties.getProperty("ForgotEmailPage.TitleIDNew"), "Title", userProfile.getTitle());
    	/*verifyAndClickWithXpath(pageProperties.getProperty("ForgotEmailPage.SelectIDNew01"), "Title");
    	verifyAndClickWithXpath(pageProperties.getProperty("ForgotEmailPage.SelectTitle").replace("Title", userProfile.getTitle()), "Title");*/
    	
    	 verifyAndInputByXpath(pageProperties.getProperty("ForgotEmailPage.FirstName"), "FirstName", userProfile.getFirstName());
  	   verifyAndInputByXpath(pageProperties.getProperty("ForgotEmailPage.SurName"), "LastName", userProfile.getLastName());
  	   verifyAndInputByXpath(pageProperties.getProperty("ForgotEmailPage.ReferenceNumber"), "PhoneNumber", userProfile.getAccNumber());
  	 Report.updateTestLog("Details is entered successfully in the application", "WARN");
	   browser.wait(500); 
  	 verifyAndClickWithXpath(pageProperties.getProperty("ForgotEmailPage.Addressbutton"), "Submit Button");
    	 	//verifyAndInputByXpath(pageProperties.getProperty("repairAndCoverPage.postCode"),"Post Code", userProfile.getTitle());
...
// Validating Enter Email Address option by comparing the Email Address displayed in the application against the Email Address fetched from the database
    ...
    public void validateEnterEmail(final UserProfile userProfile) {
        ...
        final String strTitle = userProfile.getTitle();
    ...
    public void validateEnterEmailNew(final UserProfile userProfile) {
        final String firstName = userProfile.getFirstName();
        final String lastName = userProfile.getLastName();
        final String strTitle = userProfile.getTitle();
        final String accountNum = userProfile.getAccNumber();
        ...
        final String DisplayedEmailAdd = browser.getTextByXpath(pageProperties.getProperty("ForgotEmailPage.DisplayedEmailAddressIDNew"));
        if (DisplayedEmailAdd.trim().contains(expectedEmailAdd.trim())) {
            Report.updateTestLog("Expected DB Email ID : "+expectedEmailAdd+"  Displayed Email Address Is  :" + DisplayedEmailAdd,Pass_Str);
        } else {
            Report.updateTestLog("Expected Email Address "+expectedEmailAdd+" Was Not Displayed", Fail_Str);
        }
        verifyAndClickWithXpath(pageProperties.getProperty("ForgotEmailPage.Loginbutton"),"Login to your account Link");
...
// Enter the appropriate details in the fields displayed in Forgot Email Page        
    ...
    public void enterForgotEmailDataNew(final String CustomerNumber, final String CustomerTitle, final String FirstName, final String SurName) {
        verifyAndInputByXpath(pageProperties.getProperty("ForgotEmailPage.CustomerReferenceNumberFieldID"), "CustomerReferenceNumber", CustomerNumber);
        verifyAndSelectDropDownBox(pageProperties.getProperty("ForgotEmailPage.SelectID"), "Title", CustomerTitle);
        verifyAndInputByXpath(pageProperties.getProperty("ForgotEmailPage.FirstNameFieldID"), "FirstName", FirstName);
        verifyAndInputByXpath(pageProperties.getProperty("ForgotEmailPage.SurNameFieldID"), "LastName", SurName);
