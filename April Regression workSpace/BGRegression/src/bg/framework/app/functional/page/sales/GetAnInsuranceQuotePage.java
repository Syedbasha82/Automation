...
public class GetAnInsuranceQuotePage extends BasePage {
	...
	public void enterQuoteDetails(InsuranceQuote insuranceQuote) {
		...
		verifyAndClick(pageProperties.getProperty("GetAnInsuranceQuotePage.GetAQuote"), "Get A Quote");
		//Sprint 4 Card 7553
		if(!browser.isElementVisibleWithXpath(pageProperties.getProperty("GetAnInsuranceQuotePage.GetQuoteDetails2"))){
			Report.updateTestLog("Get A Quote section have been removed", "Pass");
		}
		else{
			Report.updateTestLog("Get A Quote section is present", "Fail");
		}
	...
	private void enterBankDetails(InsuranceQuote insuranceQuote) {
		//sprint 4 Bank Details Validation included
		
		BankValidationDetailsProfile bankDetails = new TestDataHelper().BankValidationDetaisProfile("BankDetailsCount");
		
	//	for(int i=0;i<Integer.parseInt(bankDetails.getBankValidationCount());i++){
			//bankDetails = new TestDataHelper().BankValidationDetaisProfile("BankDetails"+(i+1));
		bankDetails = new TestDataHelper().BankValidationDetaisProfile("BankDetails1");
			
			browser.wait(getWaitTime());
			
			System.out.println("####################### Now in Credit card verification section #####################################");
			if(browser.isElementVisible(pageProperties.getProperty("GetAnInsuranceQuotePage.CreditCard")))
			{
				Report.updateTestLog("Credit card details present", "Fail");
			}
			else
			{
				Report.updateTestLog("Credit card details absent", "Pass");
			}
			...
			if (insuranceQuote.getPaymentType().equalsIgnoreCase("Monthly")) {
				...
				verifyAndClick(pageProperties.getProperty("GetAnInsuranceQuotePage.Monthly"), "Monthly payment");
				verifyAndInputById(pageProperties.getProperty("GetAnInsuranceQuotePage.AccountNumber"), "Account number", bankDetails.getAccountNumber());
				verifyAndInputById(pageProperties.getProperty("GetAnInsuranceQuotePage.SortCode1"), "Sortcode", bankDetails.getSortCode1());
				verifyAndInputById(pageProperties.getProperty("GetAnInsuranceQuotePage.SortCode2"), "Sortcode", bankDetails.getSortCode2());
				verifyAndInputById(pageProperties.getProperty("GetAnInsuranceQuotePage.SortCode3"), "Sortcode", bankDetails.getSortCode3());
				verifyAndInputById(pageProperties.getProperty("GetAnInsuranceQuotePage.NameOfAccountHolder"), "Account name", "Testing Team");
				verifyAndClick(pageProperties.getProperty("GetAnInsuranceQuotePage.NextButton5"), "Next Button");
				try {
					Thread.sleep(4000);
			...
			/*if(i<(Integer.parseInt(bankDetails.getBankValidationCount())-1)){
				if(browser.isElementVisibleWithXpath(".//*[@id='edit5']/span")){
					Report.updateTestLog("Bank Details passed validation", "Pass");
					browser.clickWithXpath(".//*[@id='edit5']/span");	
				}
				else{
	...
	private void enterPersonalDetails(InsuranceQuote insuranceQuote) {
		browser.wait(getWaitTime());
		if (browser.isElementVisibleWithXpath(pageProperties.getProperty("GetAnInsuranceQuotePage.Title"))){
			WebElement element = browser.getElementByXpath(pageProperties.getProperty("GetAnInsuranceQuotePage.Title"));
			element.click();
			element = browser.getElementByXpath(pageProperties.getProperty("GetAnInsuranceQuotePage.TitleOption"));
			element.click();
		}
	...
	private void caveatVerification_1() {
		...
		if(!browser.isElementVisibleWithXpath("//*[contains(text(),'The incident must have been reported')]"))
		{
			Report.updateTestLog("Caveat is Not available for [*]", "WARN");
		}else
		{
