...
public class CQSMRPage extends BasePage {
	...
	public void submitMeterReadsforImplausible() {
		...
		if (!browser.isElementVisible(pageProperties
			...
			browser.click(pageProperties
					.getProperty("CQSMR1Page.SubmitButtonOverlayID"));
		...
		public void submitMeterReadsforPlausible() {			
			...
			if (browser.isElementVisible(pageProperties
					.getProperty("CQSMR1Page.SubmitButtonOverlayID"))) {
				logInfo = " Alert OverLay is displayed for plaussible MeterRead.";
				logStatus = "FAIL";
				Report.updateTestLog(logInfo, logStatus);
				}
	...
	public void setPlausbileReadingforJIAccount(String accountNumber) {
		...
		String meterSerialNumberforfirstaccount = getSerialNUmberforJIAccountBasedonOrder(
				accountNumber, 0); 
		if (meterSerialNumberforfirstaccount.equalsIgnoreCase(meterSerialNumberforSecondaccount))
		{
		flag = true;	
		}
		logInfo = logInfo
		...
		if (dialFieldCount > 5)
		{	dialFieldCount =dialFieldCount/2;}
		// TODO: update
		...
		String previousMeterValueforSecondaccount = getPreviousMeterReadforJIAccountBasedonOrder(
				accountNumber, 1);
		 meterSerialNumberforSecondaccount = getSerialNUmberforJIAccountBasedonOrder(
				accountNumber, 1);
		...
		for (int i = dialFieldCount+1; i <=(2*dialFieldCount); i++) {
	...
	public void verifyMeterConfirmationforJIAccount(String accNumber) {
		...
		verifyIsTextPresent(currentmeterSerialNumberforsecondaccount,
				"MeterSerialNumber for first Account");
		verifyIsTextPresent("Your meter reading is complete","Your meter reading is complete");
	...
	public void clickonSMRLink(UserProfile userProfile){
		browser.open(ApplicationConfig.APP_BGB_URL+"/newJourneys/meterReading/smr1.html");		
     ...
     Report.updateTestLog(logInfo, logStatus);
      ...
      public void setPlausbileReadingforMultipleAccount(String accountNumber) {
          ...
          previousMeterValue = getPreviousMeterReadforJIAccountBasedonOrder(
                       accountNumber, 1);
          String meterSerialNumberSecond = getSerialNUmberforJIAccountBasedonOrder(
                       accountNumber, 1);
          logInfo = " The meter Serial Number for the second account is: "
                       + previousMeterValue
                       + ", The Previous Meter Read value for the second account is: "
                       + meterSerialNumberSecond;
          ...
          previousMeterValue = getPreviousMeterReadforJIAccountBasedonOrder(
                  accountNumber, 2);
     String meterSerialNumberthird = getSerialNUmberforJIAccountBasedonOrder(
                  accountNumber, 2);
     
     if (meterSerialNumberSecond.equalsIgnoreCase(meterSerialNumberthird))
		{
		flag = true;	
		}
