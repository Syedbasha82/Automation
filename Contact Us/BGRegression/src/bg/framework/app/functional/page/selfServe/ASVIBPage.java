...
public class ASVIBPage extends BasePage {
	...
	public void navigateToIdentifyFault(UserProfile userProfile) {
		...
		else
{
	...
	public void verifyAddress(UserProfile userProfile)
	{
		...
		if(browser.isTextPresent(userProfile.getHomeNumber().trim())){
			verifyIsTextPresent(userProfile.getHomeNumber().trim(),"House Number");
		}
		...
		verifyIsTextPresent(userProfile.getCountry(),"PostCode");
	}
	...
	public void verifyConfirmationPage() {
		...
		if (browser.isTextPresent(
				...
				|| browser.isTextPresent(
						"Your appointment has been rescheduled")|| browser.isTextPresent(
	...
	public void checkDateFormatBooked()
	{
		Calendar currentDate = Calendar.getInstance();
		SimpleDateFormat oDateFormat = new SimpleDateFormat("MMM yyyy");
		for (int i = 0; i <= 100; i++) {
			...
			/*String FinalDate = 	currentDate.get(currentDate.DAY_OF_MONTH)
			           				...
			           				+ oStrDate;*/
			String FinalDate = "";
			if(currentDate.get(currentDate.DAY_OF_MONTH) < 10)
			{
				 FinalDate = dayName[currentDate.get(currentDate.DAY_OF_WEEK)]
				       					.toString()
				       					+ ", 0"
				       					+ currentDate.get(currentDate.DAY_OF_MONTH)
				       					+ " "
	...
	public void payment(UserProfile userProfile) {
		verifyIsElementVisibleWithXpath(
				pageProperties.getProperty("ASVIBPayment.PopUp"), "Payment popup");
		...
		verifyAndClickWithXpath(pageProperties.getProperty("ASVIBPayment.PopUp"),
				"Payment Popup");
		browser.wait(5000);
		if(browser.isTextPresent(userProfile.getTitle()))
		{
		...
		verifyAndClickWithXpath(
				...
				"Security check submit button");
		browser.wait(5000);
		//Report.updateTestLog("Take screenshot of Alert", "WARN");
		Alert alert =  driver.switchTo().alert();				
		alert.accept();
	...
	public void navigateTrackCancelChange() {
		...
		try
		{
		Report.updateTestLog("Screenshot", "WARN");
		}
		catch(Exception ex)
		{
	...
	public void navigateTrackCancelChangeTrack() {
		browser.wait(1000);
		/*verifyIsTextPresent(
				pageProperties.getProperty("ASVIBConfirmation.TrackCancelChange"),
				"TrackCancelChange link");
	...
	public void viewDetailsCompletePage(UserProfile userProfile) {
		...
		String strAppointment = browser.getTextByXpath(pageProperties.getProperty("ASVIBViewDetails.AppointmentSection"));
		if(strAppointment.trim().contains(pageProperties.getProperty("ASVIBViewDetails.Header").trim()))
		{
			verifyIsTextPresent(pageProperties.getProperty("ASVIBViewDetails.Header"));
		}
		verifyAddress(userProfile);
		String strContact = browser.getTextByXpath(pageProperties.getProperty("ASVIBViewDetails.ContactSection"));
		if(strContact.trim().contains(pageProperties.getProperty("ASVIBViewDetails.ContactHead").trim()))
		{
	...
	public void accountSummaryChangeAppointment() {
		...
		browser.wait(1000);
		if(browser.isTextPresent(pageProperties.getProperty("ASVIBAccountSummary.Change")))
		{
			verifyAndClickWithLinkText(
					pageProperties.getProperty("ASVIBAccountSummary.Change"),
					"Change Appointment");
		}
		if(browser.isTextPresent(pageProperties.getProperty("ASVIBViewDetails.Change")))
		{
		verifyIsTextPresent(
	...
	public void accountSummarycancelAppointment() {
		...
		{
		if(verifyIsTextPresent(
				pageProperties.getProperty("ASVIBViewDetails.Cancel"),
		"View details page cancel link"))
		{
		verifyIsTextPresent(
				pageProperties.getProperty("ASVIBViewDetails.Cancel"),
				"View details page cancel link");
	...
	public void confirmCancelAppointment() {
		...
		browser.wait(1000);		
		if(browser.isElementVisible("canOverlay"))
		{
			verifyAndClick("canOverlay", "Landlord certificate cancel overlay");
		}
		browser.wait(5000);	
		verifyAndClickWithLinkText(
	...
	public void verifyPiCallErrorMessageConf() {
		browser.wait(1000);
		verifyIsTextPresent(pageProperties
				.getProperty("ASVIBNoTimeSlot.PICallErrorMessageConf"));
	}
