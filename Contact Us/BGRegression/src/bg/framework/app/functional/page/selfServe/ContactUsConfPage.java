...
public class ContactUsConfPage extends BasePage {
   ...
   // private static final String filePathAutoCef = Report.createTxtFile("AutoCef");
    ...
    public void VerifyContactUsConfirmationPage(UserProfile userprofile, String cat) {
        ...
        //if (browser.isElementVisibleWithXpath(pageProperties.getProperty("ContactUsConfirmationPage.ReferenceNo"))) {
        	if (browser.isElementVisibleWithXpath(pageProperties.getProperty("ContactUsConfirmationPage.ReferenceNoNew01"))) {
        }}catch(NoSuchElementException e){
        	//browser.isElementVisibleWithXpath(pageProperties.getProperty("ContactUsConfirmationPage.OAMReferenceNo"));
        	browser.isElementVisibleWithXpath(pageProperties.getProperty("ContactUsConfirmationPage.ReferenceNoNew01"));
        }
            ...
            //referenceNo = browser.getTextByXpath(pageProperties.getProperty("ContactUsConfirmationPage.ReferenceNo"));
            	referenceNo = browser.getTextByXpath(pageProperties.getProperty("ContactUsConfirmationPage.ReferenceNoNew01"));
            } else {
            ...
            if (!(referenceNo=="")) {
                ...
                Report.updateTestLog(strconf+"  Characters are displayed in Confirmation Page", "WARN");
   ...
   /* private void verifyAutoCefLead(String referenceNo, String text) {
        ...
        String auditDetails = dbconnector.verifyAuditDetailsContactUs(referenceNo);
        int len = auditDetails.length();
        Report.updateTestLog(auditDetails + "stored in DB**" +len+"  Characters stored in DB", "PASS");
    ...
    public void navigateToContactUsPage() {
        ...
        if (ApplicationConfig.BRAND.equalsIgnoreCase("BG")) {
        	...
        	browser.open(ApplicationConfig.APP_BG_URL+"/Contactus/");
        } else {
