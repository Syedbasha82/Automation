...
public class NectarPage extends BasePage{
	...
	public void navigateToYourPointsPage(){
		browser.wait(getWaitTime());
		browser.open(ApplicationConfig.APP_BG_URL+"/Loyalty/View-Rewards-Content/");
	...
	public void verifyYourPointsPage(){
		browser.wait(getWaitTime());
		if(browser.getTextByXpath(pageProperties.getProperty("NectarPage.YourNectarPointsText")).equalsIgnoreCase("Your Nectar points")){
			Report.updateTestLog("Your Nectar points header found in the Nectar Page", "Pass");
		}
		else{
			Report.updateTestLog("Your Nectar points header not found in the Nectar Page", "Fail");
		}
		
		if(browser.getTextByXpath(pageProperties.getProperty("NectarPage.NectarNo")).contains("98263000")){
