...
public class PredictNextBillPage  extends BasePage{
	...
	public String getTariffName(){
		String tariff = null;
		if(browser.isElementVisibleWithXpath(pageProperties.getProperty("PNB.gasTariffName"))){
		    tariff = browser.getTextByXpath(pageProperties.getProperty("PNB.gasTariffName"));
		}
		if(browser.isElementVisibleWithXpath(pageProperties.getProperty("PNB.ElecTariffName"))){
		    tariff = browser.getTextByXpath(pageProperties.getProperty("PNB.ElecTariffName"));
		}
		return tariff;
	}
	public void verifyTCRValue(String paymentOption , String energyType, String meterType,UserProfile userProfile){
		String tariff = getTariffName();
		System.out.println("99999999999999999999999999" +tariff);
		/*String postCode = browser.getTextByXpath(pageProperties.getProperty("PNB.PostCode").replace("[NUMBER]", userProfile.getAccNumber()));
		String postcode1[] = postCode.split(",");
		String Region = new TCRCalculationPage().getPostcode(postcode1[1]);*/
		String postCode = userProfile.getaddr();
		String Region = new TCRCalculationPage().getPostcode(postCode);
		System.out.println("777777777777777777777777777" +Region);
		if (energyType == "Gas"){
        	String Combination = tariff+energyType+paymentOption+Region;
        	verifyGasTCRValue(tariff,Combination);
        	verifyGasStandingChargeAndUnitRates(tariff,Combination);
        }
        if (energyType == "Elec SR"){
        	String Combination = tariff+energyType+paymentOption+Region;
        	verifyElecTCRValue(tariff,Combination);
        	verifyElecStandingChargeAndUnitRates(tariff,Combination);
        }
        if(energyType == "Dual" || energyType == "JI"){
        	String Combination = tariff+"Gas"+paymentOption+Region;
        	verifyGasTCRValue(tariff,Combination);
        	verifyGasStandingChargeAndUnitRates(tariff,Combination);
        	browser.wait(2000);
        	ArrayList<String> accounts = new ArrayList<String>();
        	accounts = browser.getFromDropBox("id", pageProperties.getProperty("PNB.AccountFromDropBox"));
        	System.out.println("999999999999999999999999999" +accounts);
        	verifyAndSelectDropDownBox(pageProperties.getProperty("PNB.AccountFromDropBox"), "Account Name", accounts.get(1));
        	System.out.println("999999999999999999999999999" +accounts.get(1));
        	verifyAndClick(pageProperties.getProperty("PNB.goButton"),"Go Button is clicked");
        	browser.wait(2000);
        	Combination = tariff+"Elec SR"+paymentOption+Region;
        	verifyElecTCRValue(tariff,Combination);
        	verifyElecStandingChargeAndUnitRates(tariff,Combination);
        }
	}
	
	public void verifyGasTCRValue(String tariff,String Combination){
		String TCRValue = new TCRCalculationPage().verifyTCRForGasTariff(tariff, Combination);
		String TCRValueOnScr = browser.getTextByXpath(pageProperties.getProperty("PNB.GasTCRValue"));
		if(TCRValue.contentEquals(TCRValueOnScr)){
			Report.updateTestLog("TCR value of gas :  Expected value:" +TCRValue+ "Displayed value:" +TCRValueOnScr , "PASS");
		}else {
			Report.updateTestLog("Error in Gas TCR value : Expected value:" +TCRValue+ "Displayed value:" +TCRValueOnScr, "FAIL");
		}
	}
	
	public void verifyGasStandingChargeAndUnitRates(String tariff,String Combination){
		PriceFinderRMR priceFinder = new TestDataHelper().getPriceFinderRMR(Combination);
		String SCOnScreen = browser.getTextByXpath(pageProperties.getProperty("PNB.GasSC"));
		String SC = priceFinder.getSC();
		if(SCOnScreen.contentEquals(SC)){
			Report.updateTestLog("Standing Charge of gas: Expected value:" +SC+ "Displayed value:" +SCOnScreen, "PASS");
		}
		else{
			Report.updateTestLog("Error in Standing charge of gas: Expected value:" +SC+ "Displayed value:" +SCOnScreen, "FAIL");
		}
		
		String unitRatesOnScr1 = browser.getTextByXpath(pageProperties.getProperty("PNB.GasUnitRates"));
		String unitRates = new TCRCalculationPage().getUnitRates(tariff, Combination);
		DecimalFormat df = new DecimalFormat("0.00");
		float f = Float.parseFloat(unitRatesOnScr1);
    	f = Float.parseFloat(df.format(f));
    	String unitRatesOnScr = String.valueOf(f);
		if(unitRatesOnScr.contentEquals(unitRates)){
			Report.updateTestLog("Unit Rates of Gas: Expected value:" +unitRates+ "Displayed value:" +unitRatesOnScr, "PASS");
		}
		else {
			Report.updateTestLog("Error in Unit Rates of Gas Expected value:" +unitRates+ "Displayed value:" +unitRatesOnScr, "FAIL");
		}
	}
	
	public void verifyElecTCRValue(String tariff,String Combination){
		DecimalFormat df = new DecimalFormat("0.00");
		...
		String TCRValueOnScr = browser.getTextByXpath(pageProperties.getProperty("PNB.ElecTCRValue"));
		if(TCRValue1.contentEquals(TCRValueOnScr)){
			Report.updateTestLog("TCR value of Electricity :  Expected value:" +TCRValue1+ "Displayed value:" +TCRValueOnScr , "PASS");
		}else {
			Report.updateTestLog("Error in Electricity TCR value : Expected value:" +TCRValue1+ "Displayed value:" +TCRValueOnScr, "FAIL");
		}
	}

	public void verifyElecStandingChargeAndUnitRates(String tariff,String Combination){
		PriceFinderRMR priceFinder = new TestDataHelper().getPriceFinderRMR(Combination);
		String SCOnScreen = browser.getTextByXpath(pageProperties.getProperty("PNB.ElecSC"));
		String SC = priceFinder.getSC();
		if(SCOnScreen.contentEquals(SC)){
			Report.updateTestLog("Standing Charge of electricity: Expected value:" +SC+ "Displayed value:" +SCOnScreen, "PASS");
		}
		else{
			Report.updateTestLog("Error in Standing charge of electricity: Expected value:" +SC+ "Displayed value:" +SCOnScreen, "FAIL");
		}
		
		String unitRatesOnScr1 = browser.getTextByXpath(pageProperties.getProperty("PNB.ElecUnitRates"));
		String unitRates = new TCRCalculationPage().getUnitRates(tariff, Combination);
		DecimalFormat df = new DecimalFormat("0.00");
		float f = Float.parseFloat(unitRatesOnScr1);
    	f = Float.parseFloat(df.format(f));
    	String unitRatesOnScr = String.valueOf(f);
		if(unitRatesOnScr.contentEquals(unitRates)){
			Report.updateTestLog("Unit Rates of electricity: Expected value:" +unitRates+ "Displayed value:" +unitRatesOnScr, "PASS");
		}
		else {
			Report.updateTestLog("Error in Unit Rates of electricity Expected value:" +unitRates+ "Displayed value:" +unitRatesOnScr, "FAIL");
		}
