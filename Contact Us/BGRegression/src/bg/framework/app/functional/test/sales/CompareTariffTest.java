...
public class CompareTariffTest extends TestBase{
	...
	private String[] DualFuelType = {"N","Y"};
	private String[] registers = {"0001" , "0002"};
		...
		for(String postCode : postCodes){
			...
			while((Tariff = compareTariff.getTariffName(TariffIncrement)) != ""){
				...
				while((pay = compareTariff.getPayment(TariffIncrement, payIncrement)) != ""){
					...
					for(String DualFuel : DualFuelType){
						for(String Registers : registers){
							...
							.verifyCompareTariffPage(Registers,Tariff, pay, postCode,DualFuel);
						}
					}
					++payIncrement;
				}
