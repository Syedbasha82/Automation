...
//        verifyPageHasNoErrors();
    ...
    public void getCordinates(String property){
    	int x = browser.locationVerificationX(property);
    	
    	int y = browser.locationVerificationY(property);
    	Report.updateTestLog("Position of the image :: X Coordinate:: "+ x +"&&&"+ " Y Coordinate:: " + y , "Done");
    }
    
    public void getImageSize(String property){
    	
    	int Width=browser.imageWidth(property);
    	int Heigth=browser.imageHeight(property);
    	//Report.updateTestLog("X Coordinate:: "+ Width + " Y Coordinate:: " + Heigth , "Done");
    }
    ...
    public void verifyAndPerformMouseOver(String name) {
        browser.wait(500);
        System.out.println("hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh");
            browser.performMouseHover();
            Report.updateTestLog(name + " object Mouse Overed ", "Done");
    }
    ...
    public void verifyAndSelectDropDownBoxByCSS(String property, String name, String value) {
		if (browser.isElementVisibleWithCss(property)) {
			browser.selectfromDropBoxByCSS(property, value);
			Report.updateTestLog(name + " field filled with the value : " + value, "Done");
		}else {
			Report.updateTestLog(name + " field not found in the application page", "FAIL");
		}
