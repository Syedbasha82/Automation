 ...
 *
 */
public class LargeBusinessGasCallBackPage extends BasePage{
 ...
 public void openLargeBusinessPage(){
	 browser.open(ApplicationConfig.APP_BG_URL+pageProperties.getProperty("LargeBusinessGas.Url"));
 ...
 public void verifyDBDetails(UserProfile userProfile){
	 OnlineDBConnector dbFunctions = new OnlineDBConnector();
		...
		String leadType[] = dbFunctions.verifyleadDBForLeadType(date,userProfile.getEmail(),"Large_Business_Gas");		
		Report.updateTestLog("Lead type is "+leadType[0] +". Hence email sent to customer", leadType[0].equalsIgnoreCase("0")?"PASS":"FAIL");
		Report.updateTestLog("Lead status is "+leadType[1] +".", leadType[1].equalsIgnoreCase("Large_Business_Gas")?"PASS":"FAIL");
		...
		String[] auditType = dbFunctions.getAuditEventTypeId(date, userProfile.getEmail(),"Large_Business_Gas");
