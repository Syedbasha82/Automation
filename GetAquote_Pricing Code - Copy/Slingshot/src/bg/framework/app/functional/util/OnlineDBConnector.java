...
public class OnlineDBConnector {
    ...
    public String DBsysdate() {
		...
		systemDate = jdbcTemplate.queryForObject(
				"select to_char(SYSTIMESTAMP,'DD-MM-YY HH12.MI.SS') from dual",
				String.class);
        return systemDate;
    }
...
public String verifyAuditEntryForCareplanCustomerdetails (String Date,String strEmail )
{
	jdbcTemplate = new SimpleJdbcTemplate(getDataSource());
	
	String Result=jdbcTemplate.queryForObject("SELECT CUSTOMER_ID FROM BGB_TA_CAREPLAN_CUST_DETAILS WHERE EMAIL_ADDRESS='"+strEmail+"'",String.class);
	System.out.println(" " +Result);
	return Result;
}
public String verifyAuditEntryForCareplanSiteAddress(String refrence )
{
	jdbcTemplate = new SimpleJdbcTemplate(getDataSource());
	
	String Result=jdbcTemplate.queryForObject("select SITE_ID FROM BGB_TA_CAREPLAN_SITE_INVOICE WHERE CUSTOMER_ID='"+refrence+"'",String.class);
	System.out.println(" " +Result);
	return Result;
}
public String verifyAuditEntryForCareplanMarketingConsent(String siteid)
{
	jdbcTemplate = new SimpleJdbcTemplate(getDataSource());
	
	String Result=jdbcTemplate.queryForObject("select SITE_ID FROM BGB_TA_CAREPLAN_SITE_INVOICE WHERE SITE_ID='"+siteid+"'",String.class);
	System.out.println(" " +Result);
	return Result;
}

public String[] verifyProductID(String siteid){
	
	jdbcTemplate = new SimpleJdbcTemplate(getDataSource());
    String query = "select PRODUCT_ID FROM BGB_TA_CAREPLAN_SITE_PRODUCTS WHERE SITE_ID='"+siteid+"'";
    List<Map<String, Object>> ss=jdbcTemplate.queryForList(query);
    System.out.println("query is:"+ss.get(0) + ss.get(1));
    String[] list=new String[20];
    int size = ss.size();
    System.out.println("qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq" +  ss.size());
    for(int i = 0 ; i<= size ; i++){
    	 System.out.println("query is:"+ss.get(i));
    	 Report.updateTestLog("Product 1 : Product ID :: " + ss.get(i), "PASS");
    }
    
 /*   if(!ss.isEmpty()){
    	for(int i = 0 ; i <= rowCount ; i++){
    	System.out.println(" Row " + i + ss.get(i));
    }
      }else{
	   System.out.println("Audit event type id is null");
         }
...
public String verifyAuditEntryForCareplanMultipleCount(String siteid)
{
      jdbcTemplate = new SimpleJdbcTemplate(getDataSource());
      
      String Result=jdbcTemplate.queryForObject("select count (*) from BGB_TA_CAREPLAN_SITE_PRODUCTS where marketing_consent='Yes' and status='S' and SITE_ID='"+siteid+"'",String.class);
      System.out.println(" " +Result);
      return Result;
}
