...
public class ForgotEmailPage extends BasePage {
    ...
    private final static String FILE_NAME = "resources/selfServe/ForgotEmail.Properties";
...
// Verify whether appropriate fields are displayed in the Forgot Email Page     
    ...
    public void validateForgotEmailTitle() {
        Integer Flg = 0;
        final String[] titlevalues = {"Please select", "Mr", "Mrs", "Ms", "Dr", "Miss", "Sir", "Reverend", "Dame", "Lady", "Lord", "Professor"};
    ...
    public void validateEnterEmail(final UserProfile userProfile) {
        ...
        if (browser.isElementVisibleWithXpath(pageProperties.getProperty("ForgotEmailPage.AccountLoginID"))) {
            ...
            if (DisplayedEmailAdd.trim().contains(expectedEmailAdd.trim())) {
                Report.updateTestLog("Displayed Email Address Is  :" + DisplayedEmailAdd,Pass_Str);
            } else {
                Report.updateTestLog("Expected Email Address Was Not Displayed", Fail_Str);
            }

            verifyAndClickWithXpath(pageProperties.getProperty("ForgotEmailPage.AccountLoginID"),"AccountLogin");
