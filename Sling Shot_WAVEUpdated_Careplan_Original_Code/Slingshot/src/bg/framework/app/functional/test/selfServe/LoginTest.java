...
//  Mandatory field : UCRN and Account Number in userprofile
    ...
    public void loginVerificationForMixedInactiveAccount() {
        Report.createTestLogHeader("Mixed customer Login Verification", "Verify that Accounts overview is displayed when a Mixed customer who has logged in into SE before 6 months ( SE accounts inactive )and the last login timestamp " +
        					"is less than 6 months tries for BG to login on BG  - BG dual account");
...
//  Mandatory field : UCRN and BGS & Elec Account Number in userprofile
    ...
    public void loginVerificationForInactiveBGSAccount() {
        Report.createTestLogHeader("Login Verification", "Verify that Accounts overview page is displayed when a  BG  only customer with last login timestamp backdated within 6 months tries to login - BGS account");
